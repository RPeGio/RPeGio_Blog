...
1. Install the theme
   ...
   <TabItem label='bun'>
   ```shell
   bun create astro@latest --template RPeGio/RPeGio_Blog
   ```
   <Aside type='tip'>
   ...
   <TabItem label='pnpm'>
   ```shell
   pnpm create astro@latest --template RPeGio/RPeGio_Blog
   ```
   </TabItem>
   <TabItem label='yarn'>
   ```shell
   yarn create astro --template RPeGio/RPeGio_Blog
   ```
   </TabItem>
   <TabItem label='npm'>
   ```shell
   npm create astro@latest -- --template RPeGio/RPeGio_Blog
   ```
   </TabItem>
   </Tabs>

   By default, this command will use the template repository’s main branch. To use a different branch name, pass it as part of the `--template` argument: `RPeGio/RPeGio_Blog#<branch>`.
...
You only need to [click fork button at theme repository](https://github.com/RPeGio/RPeGio_Blog/fork) to create your project; then clone the forked repository to your local machine.

```shell
git clone https://github.com/<your-username>/RPeGio_Blog.git
```
